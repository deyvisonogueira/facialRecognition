...
imgDey = fr.load_image_file('dey.png')
imgDey = cv2.cvtColor(imgDey, cv2.COLOR_BGR2RGB)
encodeDey = fr.face_encodings(imgDey)[0]
...
if not video.isOpened():
    ...
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            comparacao = fr.compare_faces([encodeDey], face_encoding)
            distancia = fr.face_distance([encodeDey], face_encoding)

            # Exibir os resultados no frame
            fonte = cv2.FONT_HERSHEY_DUPLEX
            rotulo = f"Deyvison? {comparacao}, Dist: {distancia[0]:.2f}"
